    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            @foreach ($trending as $tag)
                <a href="{{ route('hashtags.show', $tag->tag) }}" wire:navigate class="card-elevation p-4 hover:shadow-md transition group">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-bold text-sm text-[var(--text-primary)]">#{{ $tag->tag }}</p>
